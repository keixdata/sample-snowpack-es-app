import { Children, createContext, useContext, useRef, useEffect, useReducer, useCallback, useMemo, useImperativeHandle, createElement, forwardRef, useLayoutEffect, Component } from './react.js';
import { c as createCommonjsModule, u as unwrapExports } from './common/_commonjsHelpers-6a48b99e.js';
import { r as reactIs } from './common/index-06e991af.js';
...
function useFormikContext() {
  ...
  !!!formik ?  warning(false, "Formik context is undefined, please verify you are calling useFormikContext() as child of a <Formik> component.")  : void 0;
  return formik;
}
...
function useFormik(_ref) {
  ...
  useEffect(function () {
    {
      !(typeof isInitialValid === 'undefined') ?  warning(false, 'isInitialValid has been deprecated and will be removed in future versions of Formik. Please use initialErrors or validateOnMount instead.')  : void 0;
  ...
  var validateFormWithLowPriority = useEventCallback(function (values) {
    ...
    return scheduler_1(scheduler_2, function () {
  ...
  useEffect(function () {
    if (enableReinitialize && isMounted.current === true && !reactFastCompare(initialValues.current, props.initialValues)) {
      initialValues.current = props.initialValues;
      resetForm();
    }
  ...
  var validateField = useEventCallback(function (name) {
    ...
    if (isFunction$1(fieldRegistry.current[name].validate)) {
  ...
  var handleSubmit = useEventCallback(function (e) {
    ...
    if ( typeof document !== 'undefined') {
      ...
      if (activeElement !== null && activeElement instanceof HTMLButtonElement) {
        !(activeElement.attributes && activeElement.attributes.getNamedItem('type')) ?  warning(false, 'You submitted a Formik form using a button with an unspecified `type` attribute.  Most browsers default button elements to `type="submit"`. If this is not a submit button, please add `type="button"`.')  : void 0;
      }
    }
...
function Formik(props) {
  ...
  useEffect(function () {
    {
      !!props.render ?  warning(false, "<Formik render> has been deprecated and will be removed in future versions of Formik. Please use a child callback function instead. To get rid of this warning, replace <Formik render={(props) => ...} /> with <Formik>{(props) => ...}</Formik>")  : void 0;
    } // eslint-disable-next-line
...
function useField(propsOrFieldName) {
  ...
  {
    !formik ?  warning(false, 'useField() / <Field /> must be used underneath a <Formik> component or withFormik() higher order component')  : void 0;
  }

  !fieldName ?  warning(false, 'Invalid field name. Either pass `useField` a string or an object containing a `name` key.')  : void 0;
...
function Field(_ref) {
  ...
  useEffect(function () {
    {
      !!render ?  warning(false, "<Field render> has been deprecated and will be removed in future versions of Formik. Please use a child callback function instead. To get rid of this warning, replace <Field name=\"" + name + "\" render={({field, form}) => ...} /> with <Field name=\"" + name + "\">{({field, form, meta}) => ...}</Field>")  : void 0;
      !!(is && children && isFunction$1(children)) ?  warning(false, 'You should not use <Field as> and <Field children> as a function in the same <Field> component; <Field as> will be ignored.')  : void 0;
      !!(component && children && isFunction$1(children)) ?  warning(false, 'You should not use <Field component> and <Field children> as a function in the same <Field> component; <Field component> will be ignored.')  : void 0;
      !!(render && children && !isEmptyChildren(children)) ?  warning(false, 'You should not use <Field render> and <Field children> in the same <Field> component; <Field children> will be ignored')  : void 0;
    } // eslint-disable-next-line
...
function withFormik(_ref) {
  ...
  return function createFormik(Component$1) {
    ...
    return hoistNonReactStatics_cjs(C, Component$1 // cast type to ComponentClass (even if SFC)
    );
  };
}
...
function connect(Comp) {
  var C = function C(props) {
    return createElement(FormikConsumer, null, function (formik) {
      !!!formik ?  warning(false, "Formik context is undefined, please verify you are rendering <Form>, <Field>, <FastField>, <FieldArray>, or your custom context-using component as a child of a <Formik> component. Component name: " + Comp.name)  : void 0;
...
function (_React$Component) {
  ...
  _proto.componentDidUpdate = function componentDidUpdate(prevProps) {
    if (!reactFastCompare(getIn(prevProps.formik.values, prevProps.name), getIn(this.props.formik.values, this.props.name)) && this.props.formik.validateOnChange) {
...
function (_React$Component) {
  ...
  function FastFieldInner(props) {
    ...
    var render = props.render,
        ...
        is = props.as,
        name = props.name;
    !!render ?  warning(false, "<FastField render> has been deprecated. Please use a child callback function instead: <FastField name={" + name + "}>{props => ...}</FastField> instead.")  : void 0;
    !!(component && render) ?  warning(false, 'You should not use <FastField component> and <FastField render> in the same <FastField> component; <FastField component> will be ignored')  : void 0;
    !!(is && children && isFunction$1(children)) ?  warning(false, 'You should not use <FastField as> and <FastField children> as a function in the same <FastField> component; <FastField as> will be ignored.')  : void 0;
    !!(component && children && isFunction$1(children)) ?  warning(false, 'You should not use <FastField component> and <FastField children> as a function in the same <FastField> component; <FastField component> will be ignored.')  : void 0;
    !!(render && children && !isEmptyChildren(children)) ?  warning(false, 'You should not use <FastField render> and <FastField children> in the same <FastField> component; <FastField children> will be ignored')  : void 0;
    return _this;
  }
...
export { ErrorMessage, FastField, Field, FieldArray, Form, Formik, FormikConsumer, FormikContext, FormikProvider, connect, getActiveElement, getIn, insert, isEmptyArray, isEmptyChildren, isFunction$1 as isFunction, isInputEvent, isInteger, isNaN$1 as isNaN, isObject$1 as isObject, isPromise, isString, move, prepareDataForValidation, replace, setIn, setNestedObjectValues, swap, useField, useFormik, useFormikContext, validateYupSchema, withFormik, yupToFormErrors };
